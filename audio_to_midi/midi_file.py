...
class GenericEvent:
    ...
    def __eq__(self, other):
        ...
        if self.type == "controllerEvent":
            if (
                self.parameter1 != other.parameter1
                or self.channel != other.channel
                or self.eventType != other.eventType
            ):
                return False
        ...
        if self.type == "UniversalSysEx":
            if (
                ...
                or self.subcode != other.subcode
                or self.sysExChannel != other.sysExChannel
            ):
                return False
...
class MIDITrack:
    ...
    def addUniversalSysEx(
        self, time, code, subcode, payload, sysExChannel=0x7F, realTime=False
    ):
        """
        Add a Universal SysEx event.
        """
        self.eventList.append(
            MIDITrack.UniversalSysExEvent(
    ...
    def changeNoteTuning(
        self, tunings, sysExChannel=0x7F, realTime=False, tuningProgam=0
    ):
        ...
        self.eventList.append(
            MIDITrack.UniversalSysExEvent(0, realTime, sysExChannel, 8, 2, payload)
        )
    ...
    def writeEventsToStream(self):
        ...
        for event in self.MIDIEventList:
            if event.type == "NoteOn":
                ...
                self.MIDIdata = self.MIDIdata + struct.pack(
                    ">B", 0x03
                )  # Data length: 3
...
class MIDIFile:
    ...
    def changeNoteTuning(
        self, track, tunings, sysExChannel=0x7F, realTime=False, tuningProgam=0
    ):
        """
        ...
        this standard!
        """
        self.tracks[track].changeNoteTuning(
            tunings, sysExChannel, realTime, tuningProgam
        )
    ...
    def addUniversalSysEx(
        self, track, time, code, subcode, payload, sysExChannel=0x7F, realTime=False
    ):
        """
        Add a Universal SysEx event.
        ...
        self.tracks[track].addUniversalSysEx(
            time, code, subcode, payload, sysExChannel, realTime
        )
